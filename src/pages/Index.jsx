...
const ProfileCard = ({ username, followers, followings, imageUrl }) => {
  const cardBg = useColorModeValue("white", "gray.700");
  return (
    <Box p={5} shadow="xl" border="1px solid" borderColor={useColorModeValue("gray.800", "gray.500")} rounded="lg" bg={cardBg}>
      <Flex direction="column" align="center">
        <Avatar name={username} src={imageUrl} size="xl" mb={4} />
        <Heading as="h3" size="md" fontWeight="medium">
          {username}
        </Heading>
        <Text fontWeight="medium">Followers: {followers}</Text>
        <Text fontWeight="medium">Followings: {followings}</Text>
      </Flex>
    </Box>
  );
};
